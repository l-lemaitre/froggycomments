<?php
	class FroggyCommentsGetContentController {
		// On crée un constructeur pour récupérer les données des variables et méthodes de la classe principale du module (voir fichier froggycomments.php ligne 135)
		...
		// On déclare la méthode processConfiguration pour gérer le traitement de la soumission du formulaire de configuration
		public function processConfiguration() {
			// On utilise la fonction Tools::isSubmit pour savoir si le formulaire contenant le bouton submit_froggycomments_form a bien était envoyé (voir fichier getContent.tpl ligne 33)
			if(Tools::isSubmit('submit_froggycomments_form')) {
				// On utilise la fonction Tools::getValue pour récupérer la valeur POST associée à la clé enable_grades qu'on affecte à la variable enable_grades (voir fichier getContent.tpl lignes 13 ou 16)
				$enable_grades = Tools::getValue('enable_grades');

				$enable_comments = Tools::getValue('enable_comments');

				// On utilise la fonction Configuration::updateValue pour sauvegarder des valeurs simples dans la table de configuration de PrestaShop (ps_configuration)
				Configuration::updateValue('FROGGY_GRADES', $enable_grades);
				Configuration::updateValue('FROGGY_COMMENTS', $enable_comments);

				// On utilise la variable $this->context->smarty et sa méthode assign pour assigner une variable de confirmation à l'objet Smarty
		...
		public function renderForm() {
			...
			// On génére le formulaire en utilisant la méthode renderForm qui prend en paramètre le tableau $fields_form créé précédemment
