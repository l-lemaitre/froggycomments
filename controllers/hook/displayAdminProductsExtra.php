...
class FroggyCommentsDisplayAdminProductsExtraController
{
    ...
    public function run()
    {
        ...
        foreach ($comments as $comment) :
            // On récupére l'adresse e-mail des clients qui ont postés un ou plusieurs commentaires parmi les résultats
            // affichés (voir fichier FroggyComment.php ligne 79)
            ...
            // On affecte à la variable customersMail sa propre valeur concaténée à l'adresse e-mail du client et la
            // chaîne de caractères ", " comme séparateur (voir fichier displayAdminProductsExtra.tpl ligne 20)
            $customersMail = $customersMail . $customerMail . ", ";

            $admin_customer_link = $admin_customer_link . $this->context->link->getAdminLink('AdminCustomers') .
                '&viewcustomer&id_customer=' . (int)$customerId . ", ";
        endforeach;

        // On récupère la valeur de la variable customersMail en enlevant la chaîne de caractères ", " à la fin,
        $customersMail = Tools::substr($customersMail, 0, -2);

        $$admin_customer_link = Tools::substr($admin_customer_link, 0, -2);

        // puis avec la fonction explode on crée un tableau en utilisant la chaîne de caractères ", " comme séparateur
        // et la variable customersMail comme chaîne initiale
