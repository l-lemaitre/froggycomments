...
/* On crée le contrôleur AdminFroggyCommentsController qui étend la classe ModuleAdminController pour lister les
commentaires dans notre AdminController */
class AdminFroggyCommentsController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        $this->fields_list = array(
            ...
            'grade_display' => array('title' => $this->l('Grade'), 'align' => 'right', 'width' => 80,
                'filter_key' => 'a!grade'),
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                array('type' => 'text', 'label' => $this->l('Firstname'), 'name' => 'firstname', 'size' => 30,
                    'required' => true),
                array('type' => 'text', 'label' => $this->l('Lastname'), 'name' => 'lastname', 'size' => 30,
                    'required' => true),
                array('type' => 'text', 'label' => $this->l('email'), 'name' => 'email', 'size' => 30,
                    'required' => true),
        ...
        // On effectue la requête suivante pour récupérer le nom du produit et améliorer l’affichage de la note. Alias
        // de la classe AdminController = a, alias de la table product_lang = pl
        $this->_select = "pl.`name` as product_name, CONCAT(a.`grade`, '/5') as grade_display";
        $this->_join = 'LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (pl.`id_product` = a.`id_product` AND
        ...
        // On vérifie si la clé viewfroggy_comment existe dans les variables $_POST ou $_GET en utilisant la méthode
        // Tools::getIsset pour afficher dans le titre l’identifiant du commentaire
    ...
    // On crée la méthode protégée processBulkMyAction pour stocker les identifiants des objets sélectionnés dans la
    // variable $this->boxes
    ...
    public function renderView()
    {
        ...
        // On appelle la méthode loadProductName pour remplir la variable $product_name (voir fichier FroggyComment.php
        // lignes 71 à 78)
        ...
        $admin_delete_link = $this->context->link->getAdminLink('AdminFroggyComments') .
            '&deletefroggy_comment&id_froggy_comment=' . (int)$this->object->id;
        ...
        $admin_product_link = $this->context->link->getAdminLink('AdminProducts') . '&updateproduct&id_product=' .
            (int)$this->object->id_product . '&key_tab=ModuleFroggycomments';
        ...
        if (isset($customers[0]['id_customer'])) {
            $admin_customer_link = $this->context->link->getAdminLink('AdminCustomers') . '&viewcustomer&id_customer='
                . (int)$customers[0]['id_customer'];
        }
