    ...
    */
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'firstname' => array('type' => self::TYPE_STRING, 'validate' => 'isNameNotempty', 'size' => 20,
                'required' => true),
            'lastname' => array('type' => self::TYPE_STRING, 'validate' => 'isNameNotempty', 'size' => 20,
                'required' => true),
    ...
    /* On crée la méthode statique getProductNbComments pour récupérer le nombre de commentaires du produit
    (voir fichier comments.php ligne 36) */
    public static function getProductNbComments($id_product)
    {
        $nb_comments = Db::getInstance()->getValue('
        SELECT COUNT(`id_product`) 
        FROM `' . _DB_PREFIX_ . 'froggy_comment` 
    ...
    public static function getProductComments($id_product, $limit_start, $limit_end = false)
    {
        ...
        SELECT * FROM `' . _DB_PREFIX_ . 'froggy_comment` 
        WHERE `id_product` = ' . (int)$id_product . ' 
    ...
    public static function getCustomerNbComments($email)
    {
        $nb_comments = Db::getInstance()->getValue('
        SELECT COUNT(`id_product`) 
        FROM `' . _DB_PREFIX_ . 'froggy_comment` WHERE `email` = \'' . pSQL($email) . '\'');
    ...
    public static function getCustomerComments($email, $limit_start, $limit_end = false)
    {
        ...
        FROM `' . _DB_PREFIX_ . 'froggy_comment` pc LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (
    ...
    {
        $customerMail = Db::getInstance()->getValue('
        SELECT `email` 
    ...
    public static function getInfosOnProductsList($id_product_list)
    {
        ...
        SELECT `id_product`, AVG(`grade`) as grade_avg, count(`id_froggy_comment`) as nb_comments 
        FROM `' . _DB_PREFIX_ . 'froggy_comment` 
        WHERE `id_product` IN (' . implode(',', $id_product_list) . ') GROUP BY `id_product`');
